...
Cal_所得税 <- function(
  ...
  # 所得税用マトリクス
  income.tax.mat <- matrix(0, nrow=32, ncol=6)
  # 年金所得年額
  for (j in c(1:6)) {
    for (i in c(1:32)) {
      income.pention.mat[i,j] <- Cal_income_pention(Amount.paid.mat[[i,j]], deduction = (480000+380000))
    }
  }
  # 社会保険料(国民健康保険料)の控除
  income.pention.mat <- 
  ...
  for (j in c(1:6)) {
    for (i in c(1:32)) {
      income.tax.mat[i,j] <- as.integer(Cal_income_tax(income.pention.mat[[i,j]]))
    }
  }
  # 列名
  colnames(income.tax.mat) <- c("0ヶ月","12ヶ月","24ヶ月","36ヶ月","48ヶ月","60ヶ月")
  # tibbleへ変換
  income.tax.tib <- as_tibble(income.tax.mat,.name_repair)
  # 時系列を追加
  income.tax.tib$X.date <- seq(as.POSIXct("2025-01-01"), as.POSIXct("2056-12-31"), by = "year")
  income.tax.tib <- 
    income.tax.tib %>% 
    select(X.date,everything())
  
  # 月次データへ変換
  income.tax.month.mat <- matrix(0, nrow=32*12, ncol=7)
  for (j in c(2:7)) {
    for (i in c(1:32)) {
      ii <- (i-1)*12
      for (iii in c(1:12)) {
        ...
        income.tax.month.mat[iii,j] <- income.tax.tib[[i,j]]/12
      }
    }
  }
  # tibbleへ変換
  income.tax.month.tib <- 
    as_tibble(income.tax.month.mat, .name_repair = 'unique')
  # 列名
  colnames(income.tax.month.tib) <- c("X.date","0ヶ月","12ヶ月","24ヶ月","36ヶ月","48ヶ月","60ヶ月")  
  # 時系列を追加
  income.tax.month.tib$X.date <- seq(as.POSIXct("2024-01-01"), as.POSIXct("2055-12-31"), by = "month")
  # 累積値
  income.tax.month.tib <- 
    income.tax.month.tib %>% 
    mutate(
  ...
  # グラフ描画用ピボット変換
  income.tax.month.tib.pivot <- 
    income.tax.month.tib %>% 
    pivot_longer(cols = starts_with("cum.")) %>% 
    mutate_if(is.numeric, ~replace(., .==0, NA))
  
  # グラフ
  p.inctax <- ggplot(data = income.tax.month.tib.pivot, aes(x = X.date)) + 
    theme_bw() + 
    labs(title = "所得税累計") +
  ...
  return(income.tax.month.tib)
}

income.tax.month.tib <- Cal_所得税(Nenkin.date.kk, N.H.Insurance.mat)
...
colnames(income.tax.mat) <- c("0ヶ月","12ヶ月","24ヶ月","36ヶ月","48ヶ月","60ヶ月")
# tibbleへ変換
income.tax.tib <- as_tibble(income.tax.mat)
# 時系列を追加
income.tax.tib$X.date <- seq(as.POSIXct("2025-01-01"), as.POSIXct("2056-12-31"), by = "year")
income.tax.tib <- 
  income.tax.tib %>% 
...
# tibbleへ変換
income.tax.month.tib <- 
  as_tibble(income.tax.month.mat,.name_repair)
# 列名
colnames(income.tax.month.tib) <- c("X.date","0ヶ月","12ヶ月","24ヶ月","36ヶ月","48ヶ月","60ヶ月")  
# 時系列を追加
income.tax.month.tib$X.date <- seq(as.POSIXct("2024-01-01"), as.POSIXct("2055-12-31"), by = "month")
...
# 保存
ggsave("./PDF/所得税累計.pdf", plot = p.inctax, device = cairo_pdf, dpi=300, width=10, height=6)
